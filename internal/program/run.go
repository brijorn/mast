...
func (s *Store) startRunProcesses(state *runState, stdout, stderr io.Writer, env map[string]string) error {
	...
	if variable := strings.TrimSpace(run.StdinVariable); variable != "" {
		...
		winePrompt = strings.TrimSpace(run.StdinPrompt) != "" && strings.EqualFold(filepath.Base(run.Cmd), "winerun")
	}
	...
	processEnv := env
	if winePrompt {
		processEnv = mergeVariables(env, map[string]string{
			"MAST_STDIN_VALUE":  stdinValue,
			"MAST_STDIN_PROMPT": run.StdinPrompt,
		})
	}
