...
func WriteTouch(w io.Writer, action byte, x, y, width, height int) error {
	...
	return writeTouch(w, action, ^uint64(1), x, y, width, height, pressure)
}

func WriteTouchPointer(w io.Writer, action byte, pointerID uint64, x, y, width, height int) error {
	pressure := uint16(DefaultPressure)
	if action == ActionUp {
		pressure = 0
	}
	return writeTouch(w, action, pointerID, x, y, width, height, pressure)
}
...
func writeSwipe(w io.Writer, startX, startY, endX, endY, width, height, steps int, sleep func(time.Duration)) error {
	...
	if err := writeTouch(w, ActionDown, ^uint64(1), startX, startY, width, height, DefaultPressure); err != nil {
		return err
	}
	...
	for i := 1; i <= steps; i++ {
		...
		if err := writeTouch(w, ActionMove, ^uint64(1), x, y, width, height, DefaultPressure); err != nil {
			return err
		}
	}

	return writeTouch(w, ActionUp, ^uint64(1), endX, endY, width, height, 0)
}
...
func writeTouch(w io.Writer, action byte, pointerID uint64, x, y, width, height int, pressure uint16) error {
	buf := make([]byte, 32)
