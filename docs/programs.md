...
Mast waits until the optional prompt appears, then writes that value followed by
a newline to the main process on every start and resume. The variable must be
...
EOF. A program that omits `stdin_prompt` receives the value immediately for
