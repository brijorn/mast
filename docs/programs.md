...
closed input stream. On Linux, Mast runs that process through a pseudo-terminal
so Wine exposes the input as a Windows console handle rather than silently
turning an ordinary Unix pipe into EOF.
